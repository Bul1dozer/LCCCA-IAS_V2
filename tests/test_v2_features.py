...
BASE = os.environ.get("LCCCA_IAS_BASE_URL", "http://localhost:8000")
REQUEST_TIMEOUT = float(os.environ.get("LCCCA_IAS_REQUEST_TIMEOUT", "5"))
session = None
PASS = []
FAIL = []
...
requests = _HTTPCompat


@pytest.fixture()
def server_url():
    return os.environ.get("LCCCA_IAS_BASE_URL", BASE)
...
@pytest.mark.e2e
def test_v2_live_e2e(server_url):
    if os.environ.get("LCCCA_IAS_RUN_E2E") != "1":
        pytest.skip("Set LCCCA_IAS_RUN_E2E=1 and LCCCA_IAS_BASE_URL to run live E2E tests")
    global BASE, session, PASS, FAIL
    BASE = server_url
    ...
    if os.environ.get("LCCCA_IAS_DATABASE_URL"):
        os.environ["DATABASE_URL"] = os.environ["LCCCA_IAS_DATABASE_URL"]
    ...
    if FAIL:
        ...
        for name, detail in FAIL:
            print(f"  ✗ {name}: {detail}")
        raise AssertionError(f"{len(FAIL)} E2E checks failed")
    else:
        print("\nALL TESTS PASSED ✓")
        return
